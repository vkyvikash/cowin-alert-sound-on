...
while not done:
    try:
        ...
        print ("{}  Vaccine dila de bhagwan!".format(time.strftime('%Y-%m-%d %H:%M:%S')))
        pincodes = [(session.get('pincode'), session.get('available_capacity_dose1')) for session in sessions 
                    if session.get('min_age_limit', default_age_limit) == age_limit]
        if pincodes:
           for pincode in pincodes:
               if pincode[1] > 5:
                   print ('Pincode {} {} slots'.format(pincode[0], pincode[1]))
                   os.system('{} "{}"'.format(speech_dispatcher, pincode[0]))
